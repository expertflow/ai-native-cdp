...
**Why this matters:** Changes the folder shape for CRM-782 (already implemented on branch `crm-782-sites-tenants-restructure` in `tmp/cx-environments-cd`) and will change scope for the Phase 4 tenant-level `pre-deploy-tenant`/`deploy-tenant` work — those jobs now need to run for devops and rmt too, not just a hypothetical MTT-only site.
